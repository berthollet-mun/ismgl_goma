...
class _RecusPageState extends State<RecusPage> {
  ...
  Future<void> _loadRecentRecus() async {
    setState(() => _isLoading = true);
    final result = await _api.get('/paiements', params: {'page': 1, 'page_size': 30, 'statut': 'Validé'});
    if (result['success'] == true) {
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                const Text('ISMGL', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold, letterSpacing: 3)),
                const Text('REÇU DE PAIEMENT', style: TextStyle(color: Colors.white70, fontSize: 12)),
                const SizedBox(height: 8),
                Text(r['numero_recu'] ?? '', style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w500)),
              ],
            ),
          ),
          ...
          _buildSection('Détails du Paiement', [
            ...
            _buildInfoRow('Date', AppHelpers.formatDateTime(r['date_paiement'])),
          ...
          Container(
            ...
            child: Column(
              children: [
                const Text('MONTANT PAYÉ', style: TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(child: Text(value ?? 'N/A', style: const TextStyle(fontWeight: FontWeight.w500, fontSize: 13))),
        ],
      ),
    );
  }
  ...
  Widget _buildRecuList() {
    if (_recus.isEmpty) return const EmptyState(message: 'Aucun reçu disponible', icon: Icons.receipt_long_outlined);
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(p['nom_complet_etudiant'] ?? '', style: const TextStyle(fontWeight: FontWeight.w600)),
                      Text(p['numero_paiement'] ?? '', style: const TextStyle(color: AppTheme.primary, fontSize: 12)),
                      Text(AppHelpers.formatDate(p['date_paiement']), style: const TextStyle(color: AppTheme.textSecondary, fontSize: 11)),
                    ],
                  ),
                ),
                Column(
                  ...
                  children: [
                    ...
                    if (p['numero_recu'] != null)
                      Text(p['numero_recu'], style: const TextStyle(fontSize: 10, color: AppTheme.textSecondary)),
                  ],
                ),
              ],
            ),
